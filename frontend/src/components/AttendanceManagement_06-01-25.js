...
const AttendanceManagement = () => {
  ...
  const [tasksAddedInSession, setTasksAddedInSession] = useState(false);
  ...
  const handleClockIn = async () => {
    ...
    try {
      ...
      const clockInData = {
        employeeId,
        clockInTime, // Use the formatted date and time
        date: clockInTime.split("T")[0], // Extract the date part from the ISO string
      };
  ...
  const addTask = async () => {
    ...
    // First check stored employeeId in state
    let currentEmployeeId = employeeId;
    ...
    if (!currentEmployeeId) {
      currentEmployeeId = getUserId();
      if (currentEmployeeId) {
        setEmployeeId(currentEmployeeId); // Update state for future use
      } else {
        // If still no employee ID, show error and return
        toast.error(
          "Unable to find employee ID. Please refresh and try again."
        );
        return;
      }
    }
    ...
    try {
      ...
      if (response) {
        ...
        // Refresh tasks with the confirmed employee ID
        dispatch(
          fetchTasksAsync({
            employeeId: currentEmployeeId,
            date: currentDate,
  ...
  // Modified updateTaskStatus function with improved error handling
  ...
  const handleSubmitTasks = () => {
    const currentTaskCount = reduxTasks.length;

    // Check if no new tasks were added in this session
  ...
  return (
    <Box sx={{ maxWidth: "lg", mx: "auto", p: 3 }}>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        {clockOutData && !clockOutData.isEmergency && (
